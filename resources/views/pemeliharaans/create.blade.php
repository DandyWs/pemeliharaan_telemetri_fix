...
@section('content')
    <section class="content">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ isset($pemeliharaan) ? $url_form : url('/pemeliharaans') }}" method="POST"
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <select id="alat_telemetri_id" name="alat_telemetri_id"
                                class="form-control @error('alat_telemetri_id') is-invalid @enderror" required>
                                ...
                                @foreach ($alat as $alat)
                                    ...
                                    <option value="{{ $alat->id }}" data-jenis="{{ $alat->jenisAlat->namajenis }}"
                                        {{ old('alat_telemetri_id', isset($pemeliharaan) && $pemeliharaan->alat_telemetri_id == $alat->id) ? 'selected' : '' }}>
                ...
                /> --}}
                            <input type="date" name="tanggal" class="form-control"
                                value="{{ isset($pemeliharaan) ? $pemeliharaan->tanggal : old('tanggal') }}" required>
                            @error('tanggal')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="periode" class="form-control @error('periode') is-invalid @enderror"
                                value="{{ old('periode', isset($pemeliharaan) ? $pemeliharaan->periode : '') }}"
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="time" name="waktu" class="form-control"
                                value="{{ isset($pemeliharaan) ? $pemeliharaan->waktu : old('waktu') }}" required>
                            @error('waktu')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            <label>Cuaca</label>
                            <input type="text" name="cuaca" class="form-control @error('cuaca') is-invalid @enderror"
                                value="{{ old('cuaca', isset($pemeliharaan) ? $pemeliharaan->cuaca : '') }}"
                                placeholder="Cuaca" required />
                            @error('cuaca')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="no_alatUkur"
                                ...
                                placeholder="No Alat Ukur" required />
                            @error('no_alatUkur')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            <label>No GSM</label>
                            <input type="text" name="no_GSM" class="form-control @error('no_GSM') is-invalid @enderror"
                                value="{{ old('no_GSM', isset($pemeliharaan) ? $pemeliharaan->no_GSM : '') }}"
                                placeholder="No GSM" required />
                            @error('no_GSM')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                            ...
                            @foreach ($komponen as $komp)
                                <div class="form-group col-md-6">
                                    <label class="example" id="{{ $komp->id == 9 || $komp->id == 10 ? $komp->id : '' }}">
                                        {{ $komp->nama }}
                                        <input type="{{ $komp->id == 9 || $komp->id == 10 ? 'checkbox' : 'hidden' }}"
                                            name="chekedsetting{{ $komp->id }}" />
                                    </label>
                                    @foreach ($detailKomponen->where('komponen2_id', $komp->id) as $detail)
                                        <div class="form-check">
                                            <input type="hidden" name="detail_komponen_id[{{ $detail->id }}]"
                                                value="{{ $detail->id }}" />
                                            <input type="hidden" name="pemeliharaan2_id[{{ $detail->id }}]"
                      ...
                      /> --}}
                                            <input type="checkbox" name="cheked{{ $detail->id }}"
                                                class="form-check-input" id="detail{{ $detail->id }}"
                                                {{ old('cheked' . $detail->id, isset($pemeliharaan) && $pemeliharaan->formKomponen->where('komponen2_id', $detail->id)->first()->cheked) ? 'checked' : '' }} />
                                            <label class="form-check-label"
                                                for="detail{{ $detail->id }}">{{ $detail->namadetail }}</label>
                                        </div>
                                    @endforeach
                                    @if ($komp->id == 7)
                                        {{-- <div class="form-group col-md-6"> --}}
                                        <div class="row">
                                            ...
                                            <div class="col-md-6">
                                                <input type="text" name="tegangan"
                                                    class="form-control @error('tegangan') is-invalid @enderror"
                                                    placeholder="Tegangan" required />
                                                @error('tegangan')
                                                    <span class="error invalid-feedback">{{ $message }}</span>
                                                @enderror
                                            </div>
                        ...
                        <div class="row example" id="form-setting" style="display: none;">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <table class="table table-bordered text-center">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td><input type="text" name="simulasi_sebelum" class="form-control" />
                                            </td>
                                            <td><input type="text" name="display_sebelum" class="form-control" /></td>
                                            <td><input type="text" name="simulasi_sesudah" class="form-control" />
                                            </td>
                                ...
                                <table class="table table-bordered text-center">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><input type="text" name="display_aktual_sesudah"
                                                    class="form-control" /></td>
                                        </tr>
                                    </tbody>
                                </table>
                        ...
                        <div class="form-group col-md-12" style="margin-top: 20px;">
                            ...
                            <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror">{{ old('keterangan', isset($pemeliharaan) ? $pemeliharaan->keterangan : '') }}</textarea>
                            @error('keterangan')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mt-3">
                            <button class="btn btn-sm btn-success btn-submit">Simpan</button>
